...
CREATE POLICY "banned_users_insert" ON public.banned_users
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.users WHERE student_id = banned_users.staff_id AND role IN ('mod', 'media_admin', 'staff'))
  );

-- Update RPC for moderation delete
...
BEGIN
  ...
  IF v_role IS NULL OR v_role NOT IN ('mod', 'media_admin', 'staff') THEN
