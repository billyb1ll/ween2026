...
export function VibeCheckPage() {
  ...
  const [animatingDir, setAnimatingDir] = useState<
    "left" | "right" | "rewind-left" | "rewind-right" | null
  >(null);
      ...
      try {
        ...
        if (data) {
          const mapped: Profile[] = data.map((u) => ({
            ...
            faculty: u.faculty || "Baan 7",
            major: u.major || "Freshman",
            bio: u.ig
              ? `IG: @${u.ig}`
              : "Excited to attend Baan 7 Orientation! 🚀",
            images:
  ...
  const handleSwipe = useCallback(
    (direction: "left" | "right") => {
      ...
      setSwipedHistory((prev) => [
        ...prev,
        { profileId: profile.id, direction },
      ]);
      ...
      if (shouldReduceMotion) {
        ...
        if (isMatch) {
          ...
          const newConfetti = Array.from({ length: 30 }).map((_, i) => ({
            ...
            color: ["#496268", "#7c563f", "#fdcaad", "#c5e0e6", "#c0392b"][
              i % 5
            ],
  ...
  const handleRewind = useCallback(() => {
    ...
    setAnimatingDir(
      lastSwipe.direction === "right" ? "rewind-right" : "rewind-left",
    );
    setCurrentIndex((prev) => prev - 1);
  ...
  const handleSendMessage = () => {
    ...
    if (name.includes("Elena")) {
      replyText =
        "Hey! I'm actually just drawing some designs right now. Do you want to go grab a coffee at the campus café tomorrow? ☕";
    } else if (name.includes("Marcus")) {
      replyText =
        "Hey there! Always down for a new coding partner or jamming together. What kind of music are you into? 🎸";
    } else if (name.includes("Sophia")) {
      replyText =
        "Hi! Nice to meet you. I'm sketching near the main gallery. We should definitely explore the art scene together! 🎨";
    } else if (name.includes("Kai")) {
      replyText =
        "Hey! Awesome to connect. If you love the outdoors, we should definitely hit some trails soon! 🌿";
    } else if (name.includes("Bell") || matchedProfile.isStaff) {
      replyText =
        "Welcome to orientation! 🧡 I'm so excited to guide you and answer any questions you have about campus life or housing. Let's make this year amazing!";
    }
            ...
            >
              <motion.div
                ...
                style={{
                  ...
                  height: "100%",
                  cursor:
                    shouldReduceMotion || !!matchedProfile ? "auto" : "grab",
                }}
                ...
                initial={
                  animatingDir === "rewind-left"
                    ? { x: -300, opacity: 0, rotate: -15 }
                    : animatingDir === "rewind-right"
                      ? { x: 300, opacity: 0, rotate: 15 }
                      ...
                      {currentProfile.images.map((_, idx) => (
                        ...
                        >
                          <Box
                            ...
                            bg="white"
                            transition={
                              idx === activePhotoIndex ? "width 0.2s" : "none"
                            }
                          />
                        </Box>
                      ))}
                  ...
                  >
                    <VStack align="start" gap={1} mb={2} pointerEvents="auto">
                      <Heading
                        ...
                        lineHeight={1.2}
                      >
                        {currentProfile.name}, {currentProfile.faculty}
                      </Heading>
                      <Text
                        color={{
                          base: "var(--c-chocolate-light)",
                          md: "var(--c-chocolate)",
                        }}
                        fontWeight="600"
                        fontSize="sm"
            ...
            >
              ...
              <VStack gap={1} textAlign="center">
                ...
                <Text fontSize="sm" color="fg.subtle" maxW="280px">
                  You have swiped on all available profiles. Reset the deck to
                  browse again!
                </Text>
              </VStack>
              <Button
                    ...
                    >
                      <Box
                        ...
                        borderTopRightRadius={
                          msg.sender === "user" ? "4px" : "2xl"
                        }
                        borderTopLeftRadius={
                          msg.sender === "match" ? "4px" : "2xl"
                        }
                      ...
                      >
                        <HStack gap={1.5}>
                          <Box
                            ...
                            borderRadius="full"
                            bg="fg.subtle"
                            style={{ animation: "breath 1s infinite" }}
                          />
                          <Box
                            w="6px"
                            h="6px"
                            borderRadius="full"
                            bg="fg.subtle"
                            style={{ animation: "breath 1s infinite 0.2s" }}
                          />
                          <Box
                            w="6px"
                            h="6px"
                            borderRadius="full"
                            bg="fg.subtle"
                            style={{ animation: "breath 1s infinite 0.4s" }}
                          />
                        </HStack>
                      </Box>
                    </Flex>
