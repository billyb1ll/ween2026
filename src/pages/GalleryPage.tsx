import { Box, Flex, Heading, HStack, SimpleGrid, Text, VStack, Image, Button, Input, Spinner } from '@chakra-ui/react'
...
export function GalleryPage() {
  ...
  return (
    ...
    >
      {/* Page Header */}
      <VStack gap={2} mb={{ base: 6, md: 12 }} animation="fade-in-up 0.6s var(--ease-out-expo) both">
        <Heading
          as="h1"
          fontFamily="heading"
          ...
          <Button
            ...
            onClick={() => {
              if (!user) {
                toaster.create({
                  title: 'Sign In Required',
                  description: 'Please sign in or register to upload photos to the gallery.',
                  type: 'warning',
                })
                return
              }
      ...
      {showUploadForm && user && (
        ...
        >
          <VStack gap={4} as="form" onSubmit={handleUploadPhoto}>
            <Heading size="xs" color="var(--c-chocolate)" fontFamily="'Playfair Display', serif" fontSize="lg" fontWeight="700">
              Share a New Memory
            </Heading>
      ...
      {loading ? (
        ...
        <Flex
          ...
          borderRadius="2xl"
        >
          <Text color="fg.subtle">No photos uploaded yet. Be the first to share a memory!</Text>
        </Flex>
      ) : (
...
function GalleryCard({ img, index, isHovered, onHoverStart, onHoverEnd, onLike }: GalleryCardProps) {
  return (
    <Box
    ...
    >
      {/* Image container */}
      <Box h={{ base: '220px', sm: '240px', md: index % 3 === 0 ? '320px' : '260px' }}>
        <Image
          src={img.src}
          alt={img.caption}
      ...
      >
        ...
        <Flex justify="space-between" align="center" mt={1}>
          ...
          <Button
            ...
            cursor="pointer"
          >
            <Box className="material-symbols-outlined" fontSize="sm" style={{ fontVariationSettings: "'FILL' 1" }}>
              favorite
            </Box>
