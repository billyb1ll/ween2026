  ...
  useEffect(() => {
    ...
    kpiChannel
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'posts' }, (payload) => {
        setKpi((prev) => {
          if (!prev) return prev;
          const diff = payload.eventType === 'INSERT' ? 1 : payload.eventType === 'DELETE' ? -1 : 0;
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'collected_cards' }, (payload) => {
        setKpi((prev) => {
          if (!prev) return prev;
          const diff = payload.eventType === 'INSERT' ? 1 : payload.eventType === 'DELETE' ? -1 : 0;
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'live_chats' }, (payload) => {
        setKpi((prev) => {
          if (!prev) return prev;
          const diff = payload.eventType === 'INSERT' ? 1 : payload.eventType === 'DELETE' ? -1 : 0;
      ...
      <VStack align="stretch" gap={8}>
        <Flex justify="space-between" align="center" wrap="wrap" gap={4}>
          <Box>
            ...
            <Text color="fg.subtle" fontSize="sm">Real-time metrics and system health.</Text>
          </Box>
        </Flex>
        ...
        {loading ? (
          ...
          <VStack align="stretch" gap={8}>
            <OnboardingProgress
              registeredStudents={kpi.registeredStudents}
              totalStudents={kpi.totalStudents}
              rate={onboardingRate}
            />
            <KPIGrid kpi={kpi} />
            <EngagementSection kpi={kpi} engagementIndex={engagementIndex} />
          </VStack>
        ) : (
      ...
      (ctx) => {
        ...
        if (barRef.current) {
          gsap.fromTo(barRef.current,
            { width: '0%' },
            { width: `${rate}%`, duration: reduceMotion ? 0 : 1.2, ease: 'power3.out', delay: 0.3 }
          );
        }
      }
    );
  ...
  return (
    ...
    >
      <Flex justify="space-between" align="center" mb={3} wrap="wrap" gap={2}>
        <Box>
          <Text fontSize="sm" fontWeight="700" color="var(--c-lagoon)">Freshmen Onboarding Progress</Text>
          <Text fontSize="xs" color="fg.subtle">Registered students vs total whitelisted</Text>
        </Box>
        <Badge colorPalette="teal" fontSize="sm" py={1} px={3} borderRadius="full">
          {rate.toFixed(1)}% Registered
        </Badge>
      </Flex>
      <Box w="100%" h="16px" bg="rgba(73, 98, 104, 0.08)" borderRadius="full" overflow="hidden" position="relative">
        <Box ref={barRef} h="100%" bg="var(--c-lagoon)" borderRadius="full" style={{ width: 0 }} />
      </Box>
      <Flex justify="space-between" mt={2} fontSize="xs" color="fg.subtle" fontWeight="600">
        <Text>{registeredStudents} Registered</Text>
        <Text>{totalStudents} Whitelisted Students</Text>
      </Flex>
    </Box>
  );
}

function KPIGrid({ kpi }: { kpi: KPIData }) {
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (ctx) => {
        const { reduceMotion } = ctx.conditions as { reduceMotion: boolean; motion: boolean };
        if (gridRef.current) {
          const cards = gridRef.current.querySelectorAll('[data-kpi-card]');
          gsap.from(cards, {
            y: reduceMotion ? 0 : 24,
  ...
  const cards = [
    { title: 'Total Registered Users', value: kpi.totalUsers, icon: 'group', color: 'var(--c-lagoon)' },
    { title: 'Total Students', value: kpi.totalStudents, icon: 'school', color: 'var(--c-lagoon)' },
    { title: 'Staff & Moderators', value: kpi.totalStaff, icon: 'shield_person', color: 'var(--c-lagoon)' },
    { title: 'Whitelisted Guests', value: kpi.totalWhitelisted, icon: 'how_to_reg', color: 'var(--c-orange)' },
    { title: 'Total Posts', value: kpi.totalPosts, icon: 'post_add', color: 'brand.900' },
    { title: 'Total Comments', value: kpi.totalComments, icon: 'forum', color: 'brand.900' },
    { title: 'Vibe Checks', value: kpi.totalVibeChecks, icon: 'verified', color: 'green.600' },
    { title: 'Chat Volume', value: kpi.totalChats, icon: 'chat_bubble', color: 'var(--c-orange)' },
  ];

  return (
    <SimpleGrid ref={gridRef} columns={{ base: 1, sm: 2, md: 3, lg: 4 }} gap={6}>
      {cards.map((c) => (
...
function EngagementSection({ kpi, engagementIndex }: { kpi: KPIData; engagementIndex: number }) {
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (ctx) => {
        const { reduceMotion } = ctx.conditions as { reduceMotion: boolean; motion: boolean };
        if (countRef.current && !reduceMotion && engagementIndex > 0) {
          const obj = { val: 0 };
          gsap.to(obj, {
            val: engagementIndex,
            duration: 1.4,
      ...
      <SimpleGrid columns={{ base: 1, md: 2 }} gap={8} alignItems="center">
        <VStack align="stretch" gap={4}>
          ...
          <Text fontSize="xs" color="fg.subtle" maxW="60ch">
            Measures all active touchpoints: live chat, board posts, comments, and completed VibeQuest card checks.
          </Text>
        </VStack>
        <VStack align="stretch" gap={3}>
          ...
          <EngagementBar label="Vibe Checks" value={kpi.totalVibeChecks} max={engagementIndex} color="green.600" />
        </VStack>
      </SimpleGrid>
    </Box>
  );
...
function KPICard({ title, value, icon, color }: { title: string; value: number; icon: string; color: string }) {
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (ctx) => {
        const { reduceMotion } = ctx.conditions as { reduceMotion: boolean; motion: boolean };
        if (valRef.current && !reduceMotion && value > 0) {
          const obj = { val: 0 };
          gsap.to(obj, {
            val: value,
            duration: 1,
            ...
            onUpdate() {
              if (valRef.current) {
                valRef.current.textContent = Math.round(obj.val).toLocaleString();
              }
            },
          });
        }
  ...
  return (
    ...
    >
      <Flex align="center" justify="space-between" mb={3}>
        {/* No uppercase/tracked eyebrow — sentence case at xs weight conveys hierarchy cleanly */}
        <StatLabel fontSize="xs" fontWeight="600" color="fg.subtle">{title}</StatLabel>
        <Box
          w="36px"
          h="36px"
          borderRadius="lg"
      ...
      <StatValueText ref={valRef} fontSize="2xl" fontWeight="800" color="var(--c-ink)" fontFamily="heading">
    ...
    mm.add(
      ...
      (ctx) => {
        ...
        if (barRef.current) {
          gsap.fromTo(barRef.current,
            { width: '0%' },
            { width: `${percentage}%`, duration: reduceMotion ? 0 : 1, ease: 'power3.out', delay: reduceMotion ? 0 : 0.4 }
          );
        }
      }
    );
  ...
  return (
    <Box>
      ...
      <Box w="100%" h="8px" bg="rgba(0,0,0,0.04)" borderRadius="full" overflow="hidden">
        <Box ref={barRef} h="100%" bg={color} borderRadius="full" style={{ width: 0 }} />
      </Box>
    </Box>
  );
}
